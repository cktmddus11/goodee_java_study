...
try{
	...
	Elements e1 = doc.select(".tl2.bdl");
	Elements e2 = doc.select(".tc");
	for(Element ele : e1){
		list.add(ele.html()); // 클래스가 tc인 태그 내용들만 가져와서 리스트에 넣
	}
	for(Element ele : e2){
		list2.add(ele.html()); // 클래스가 tc인 태그 내용들만 가져와서 리스트에 넣
	}
}catch(Exception e){
	e.printStackTrace();
}

pageContext.setAttribute("list2", list); // 코드
pageContext.setAttribute("list", list2); // 나라
%><table>
<c:forEach items = "${list}" var = "v" varStatus = "stat">
	<c:choose>
		<c:when test="${stat.index % 7 == 0}"><%-- 통화코드 --%>
			<tr><td rowspan="6">${v}</td>
			<td rowspan="6">${list2[stat.index/7]}
		</c:when>
		<c:when test="${stat.index % 7 == 1 }"> <!--  바로 다음칸에 tr닫아주려고 -->
			<td> ${v}</td></tr>
		</c:when>
		<c:otherwise>
			<tr><td>${v}</td></tr>
