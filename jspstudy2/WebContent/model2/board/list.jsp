		...
		<c:if test="${boardcnt > 0 }">
			...
			<c:forEach var="b" items="${list}">
				<tr>
					...
					<td style="text-align: left">
						<!--  첨부파일 표시 --> <c:if test="${!empty b.file1}">
							<a href="file/${b.file1}" style="text-decoration: none;">@</a>
						</c:if> <c:if test="${empty b.file1}">
						&nbsp;&nbsp;&nbsp;
					</c:if> <!--  답글 표시하기 --> <c:if test="${b.grplevel > 0}">
							<c:forEach begin="1" end="${b.grplevel}">
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						</c:forEach>└
					</c:if> <!--  EL에 쓰는거 property가져옴 (get 안쓴다는거) --> <a
						href="info.do?num=${b.num}">${b.subject}</a>
					</td>
					<td>${b.name}</td>
			...
			<tr>
				<td colspan="5">
					<!-- 이전 --> 
					<c:if test="${pageNum <= 1}">
							<div class="w3-button w3-black">[이전]</div>
						</c:if> <c:if test="${pageNum > 1}">
						<a href="javascript:listdo(${pageNum-1})" class="w3-button w3-black">[이전]</a>
					</c:if> 
					<!-- 번호--> 
					<c:forEach begin="${startpage}" end="${endpage}" var="a">
						<c:if test="${a == pageNum }">
									<div class="w3-button w3-black">[${a}]</div> 
								</c:if>
						<c:if test="${a != pageNum }">
							<a href="javascript:listdo(${a})" class="w3-button w3-black">[${a}]</a>
						</c:if>
					</c:forEach> <!-- 다음 --> 
						<c:if test="${pageNum >= maxpage}">
							<div class="w3-button w3-black">[다음]</div>
						</c:if> 
						<c:if test="${pageNum < maxpage}">
						<a href="javascript:listdo(${pageNum + 1})" class="w3-button w3-black">[다음]</a>
					</c:if>
				</td>
			</tr>
		</c:if>
